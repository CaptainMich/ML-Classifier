...
class Linear_SVC(object):
    """This class performs Linear SVC algorithm
    """
    ...
    def train(self):
        ...
        self.model = self.svc_model.fit(self.data_train, self.label_train)
    ...
    def plot_precision_recall_f1(self, classes=['Won','Loss'], display=False):
        ...
        visualizer.poof(outpath=self.cfg['plot_path'] + "linear-svc-report.png") # save the data
        if display:
    ...
    def confusion_matrix(self, classes=['Won','Loss'], display=False):
        ...
        for title, normalize in titles_options:
            ...
            plt.savefig(self.cfg['plot_path'] + f"linear-svc-confusion-matrix{text}.png")
    ...
    def plot_decision_boundary(self, 
        ...
        # we fit the data using the Linear SVC Classifier
        self.svc_model.fit(x_mat, y_mat)
        ...
        Z = self.svc_model.predict(np.c_[xx.ravel(), yy.ravel()])
        ...
        plt.savefig(self.cfg['plot_path'] + "linear-svc-decision-boundary.png")
    ...
    def generate_report(self):
        ...
        pdf.add_page()
        pdf.set_xy(20, 5)
        pdf.set_title("LinearSVC")
        pdf.accept_page_break()

        # Title 
        pdf.set_font('arial', 'B', 25)
        pdf.cell(0, 30, 'Linear SVC', border=0, ln=2, align='C')
        ...
        # Plot Report
        pdf.cell(10)
        pdf.image(self.cfg['plot_path'] + "linear-svc-report.png", x = 30, y = 40, w = 150, h = 100, type = '', link = '')
        ...
        pdf.cell(0)
        pdf.image(self.cfg['plot_path'] + "linear-svc-confusion-matrix.png", x = 20, y = 160, w = 100, h = 75, type = '', link = '')  
        pdf.cell(10)
        pdf.image(self.cfg['plot_path'] + "linear-svc-confusion-matrix-normalize.png", x = 100 , y = 160, w = 100, h = 75, type = '', link = '')
        ...
        pdf.image(self.cfg['plot_path'] + "linear-svc-decision-boundary.png", x = 20, y = 30, w = 180, h = 120, type = '', link = '')
        pdf.output(self.cfg['info_path'] + 'linear-svc-report.pdf', 'F')